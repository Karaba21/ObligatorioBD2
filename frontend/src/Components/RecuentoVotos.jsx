...
const RecuentoVotos = () => {
  ...
  const cargarRecuento = async () => {
    try {
      ...
      const response = await fetch('http://localhost:5001/api/recuento-votos');
  ...
  return (
    <div className="recuento-bg">
      <div className="recuento-barra">
        <span className="recuento-sesion">Sesion Iniciada</span>
        <button className="recuento-cerrar" onClick={handleCerrarSesion}>
          Cerrar Sesion
        </button>
      </div>
      ...
      <div className="recuento-container">
        <div className="recuento-header">
          ...
          <p className="recuento-subtitulo">Elecciones Generales 2024</p>
        </div>

        <div className="recuento-stats">
          <div className="recuento-stat-card">
            <h3>Total de Votantes</h3>
            <span className="recuento-stat-numero">{recuento?.totalVotantes?.toLocaleString()}</span>
          </div>
          <div className="recuento-stat-card">
            <h3>Votos Emitidos</h3>
            <span className="recuento-stat-numero">{recuento?.totalVotos?.toLocaleString()}</span>
          </div>
          <div className="recuento-stat-card">
            <h3>Participación</h3>
            <span className="recuento-stat-numero">{recuento?.participacion}%</span>
          </div>
        </div>

        <div className="recuento-resultados">
          <h2 className="recuento-seccion-titulo">Resultados por Lista</h2>
          <div className="recuento-partidos">
            {recuento?.votosPorLista?.map((lista) => (
              <div key={lista.numeroLista} className="recuento-partido-card">
                <div className="recuento-partido-info">
                  <h3 className="recuento-partido-nombre">
                    Lista {lista.numeroLista} - {lista.nombrePartido}
                  </h3>
                  <div className="recuento-partido-votos">
                    <span className="recuento-votos-numero">{lista.votos.toLocaleString()}</span>
                    <span className="recuento-votos-porcentaje">({lista.porcentaje}%)</span>
                  </div>
                </div>
        ...
        <div className="recuento-footer">
          ...
          <button className="recuento-btn-volver" onClick={handleVolver}>
